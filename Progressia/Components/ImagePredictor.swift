...
class ImagePredictor
{
    ...
    static func createImageClassifier() -> VNCoreMLModel
    {
        ...
        let imageClassifierWrapper = try? HappyImageClassifier(configuration: defaultConfig)

        guard let imageClassifier = imageClassifierWrapper
        else
        {
            fatalError("Error: App failed to create an image classifier model instance")
        }
        ...
        guard let imageClassifierVisionModel = try? VNCoreMLModel(for: imageClassifierModel)
        else
        {
            fatalError("Error: App failed to create a `VNCoreMLModel` instance.")
        }
    ...
    struct Prediction
    {
        ...
        /// The image classifier's confidence as a double.
        let confidence: Double
        
        static let predictionWeight =
        [
            "Happy"     : 1.0,
            "Neutral"   : 0.3,
            "Angry"     : -0.15,
    ...
    func makePredictions(for photo: UIImage, completionHandler: @escaping ImagePredictionHandler) throws
    {
        ...
        guard let photoImage = photo.cgImage
        else
        {
            fatalError("Error: Photo doesn't have underlying CGImage.")
        }
    ...
    private func visionRequestHandler(_ request: VNRequest, error: Error?)
    {
        ...
        guard let predictionHandler = predictionHandlers.removeValue(forKey: request)
        else
        {
            fatalError("Error: Every request must have a prediction handler., \(error?.localizedDescription ?? "")")
        }
